...
public class GuiaDespachoService {
    ...
    public GuiaDespachoResponse crearGuia(GuiaDespachoRequest request) throws Exception {
        ...
        awsS3Service.crearBucketSiNoExiste(bucket);
    ...
    /**
     * Crea guías de demostración desde el mismo código para que el proyecto pueda
     * probarse como microservicio sin preparar archivos manualmente.
     */
    public List<GuiaDespachoResponse> crearGuiasDemo() throws Exception {
        List<GuiaDespachoRequest> datosDemo = List.of(
                new GuiaDespachoRequest("PED-1001", "transportistaX", "transportistaX", "Comercial Los Andes",
                        "Av. Providencia 1234, Santiago", "10 cajas de repuestos"),
                new GuiaDespachoRequest("PED-1002", "transportistaNorte", "transportistaNorte", "Ferretería Norte",
                        "Ruta 5 Norte Km 470, La Serena", "5 pallets de materiales"),
                new GuiaDespachoRequest("PED-1003", "transportistaSur", "transportistaSur", "Bodega Sur",
                        "Av. Alemania 980, Temuco", "Equipamiento tecnológico")
        );

        List<GuiaDespachoResponse> respuestas = new ArrayList<>();
        for (GuiaDespachoRequest request : datosDemo) {
            respuestas.add(crearGuia(request));
        }
        return respuestas;
    }

    public List<S3ObjectDto> consultarHistorialDemo() {
        String fecha = LocalDate.now().format(FORMATTER);
        return consultarHistorial(fecha, "transportistaX");
    }
