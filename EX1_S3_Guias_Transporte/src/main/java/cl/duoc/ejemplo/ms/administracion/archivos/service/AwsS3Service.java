...
public class AwsS3Service {
	...
	/**
	 * Crea el bucket si no existe. Permite que el microservicio quede listo
	 * automáticamente para la pauta, sin crear el bucket manualmente en consola AWS.
	 */
	public void crearBucketSiNoExiste(String bucket) {
		try {
			s3Client.headBucket(HeadBucketRequest.builder().bucket(bucket).build());
			log.info("Bucket S3 disponible: {}", bucket);
		} catch (NoSuchBucketException e) {
			log.info("Bucket S3 no existe. Creando bucket: {}", bucket);
			s3Client.createBucket(CreateBucketRequest.builder().bucket(bucket).build());
			log.info("Bucket S3 creado correctamente: {}", bucket);
		} catch (S3Exception e) {
			if (e.statusCode() == 404) {
				log.info("Bucket S3 no existe. Creando bucket: {}", bucket);
				s3Client.createBucket(CreateBucketRequest.builder().bucket(bucket).build());
			...
			if (e.statusCode() == 403) {
				throw new S3AccessDeniedException("validar o crear el bucket: " + bucket, e);
			}
			throw new S3OperationException("Error al validar o crear el bucket: " + bucket, e);
		}
	}
	...
	public List<S3ObjectDto> listObjectsByPrefix(String bucket, String prefix) {
		...
		try {
			...
			if (e.statusCode() == 403) {
				throw new S3AccessDeniedException("consultar historial del bucket: " + bucket, e);
			}
			throw new S3OperationException("Error al consultar historial en S3", e);
		}
	}
