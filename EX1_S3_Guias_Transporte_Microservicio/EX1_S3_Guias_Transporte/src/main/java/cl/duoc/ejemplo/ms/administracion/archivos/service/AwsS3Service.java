...
public class AwsS3Service {
	...
	public List<S3ObjectDto> listObjects(String bucket) {
		...
		try {
			...
			log.warn("No se pudo listar en AWS S3. Se usará almacenamiento local del microservicio. Motivo: {}", e.getMessage());
			return listLocalObjects(bucket, "");
		}
	}

	/**
	 * Lista objetos en S3 filtrando por prefijo/carpeta.
	 */
	public List<S3ObjectDto> listObjectsByPrefix(String bucket, String prefix) {
		...
		try {
			...
			log.warn("No se pudo consultar historial en AWS S3. Se usará almacenamiento local del microservicio. Motivo: {}", e.getMessage());
			return listLocalObjects(bucket, prefix);
		}
	}

	/**
	 * Sube un arreglo de bytes a S3.
	 */
	public void uploadBytes(String bucket, String key, byte[] bytes, String contentType) {
		...
		try {
			...
			log.warn("No se pudo subir a AWS S3. Se guardará en almacenamiento local del microservicio. Motivo: {}", e.getMessage());
			saveLocalObject(bucket, key, bytes);
		}
	}
	...
	public byte[] downloadAsBytes(String bucket, String key) {
		...
		try {
			...
			log.warn("No se pudo descargar desde AWS S3. Se buscará en almacenamiento local del microservicio. Motivo: {}", e.getMessage());
	...
	public void upload(String bucket, String key, MultipartFile file) {
		...
		try {
			...
			try {
				log.warn("No se pudo subir a AWS S3. Se guardará en almacenamiento local del microservicio. Motivo: {}", e.getMessage());
				saveLocalObject(bucket, key, file.getBytes());
			} catch (IOException ioException) {
				throw new S3UploadException("Error al leer el archivo: " + ioException.getMessage(), ioException);
			}
		}
	}
	...
	public void deleteObject(String bucket, String key) {
		...
		try {
			...
			log.warn("No se pudo eliminar desde AWS S3. Se eliminará desde almacenamiento local del microservicio. Motivo: {}", e.getMessage());
			deleteLocalObject(bucket, key);
		}
	}

	private void saveLocalObject(String bucket, String key, byte[] bytes) {
		try {
			Path path = localObjectPath(bucket, key);
			Files.createDirectories(path.getParent());
			Files.write(path, bytes);
			log.info("Archivo guardado localmente como respaldo S3: {}", path);
		} catch (IOException e) {
			throw new S3UploadException("Error al guardar archivo local del microservicio", e);
		}
	}

	private byte[] readLocalObject(String bucket, String key) {
		try {
			Path path = localObjectPath(bucket, key);
			if (!Files.exists(path)) {
				throw new S3ObjectNotFoundException(key, bucket);
			}
			return Files.readAllBytes(path);
		} catch (IOException e) {
			throw new S3OperationException("Error al leer archivo local del microservicio", e);
		}
	}

	private void deleteLocalObject(String bucket, String key) {
		try {
			Files.deleteIfExists(localObjectPath(bucket, key));
		} catch (IOException e) {
			throw new S3OperationException("Error al eliminar archivo local del microservicio", e);
		}
	}

	private List<S3ObjectDto> listLocalObjects(String bucket, String prefix) {
		try {
			Path root = localBucketPath(bucket);
			...
			try (var stream = Files.walk(root)) {
				return stream.filter(Files::isRegularFile)
						.map(root::relativize)
						.map(Path::toString)
						.map(key -> key.replace(File.separatorChar, '/'))
						.filter(key -> prefix == null || prefix.isBlank() || key.startsWith(prefix))
						.sorted(Comparator.naturalOrder())
						.map(key -> {
							Path file = localObjectPath(bucket, key);
							try {
								return new S3ObjectDto(key, Files.size(file), Files.getLastModifiedTime(file).toInstant().toString());
							} catch (IOException e) {
								return new S3ObjectDto(key, 0L, Instant.now().toString());
							}
						})
						.collect(Collectors.toList());
			}
		} catch (IOException e) {
			throw new S3OperationException("Error al listar archivos locales del microservicio", e);
		}
	}

	private Path localObjectPath(String bucket, String key) {
		return localBucketPath(bucket).resolve(key.replace("/", File.separator));
	}

	private Path localBucketPath(String bucket) {
		return Path.of(efsPath, "s3", bucket);
	}
}
