...
public class GuiaDespachoService {
    ...
    public GuiaDespachoResponse actualizarGuiaGenerada(String fecha, String transportista, String idGuia, GuiaDespachoRequest request) throws Exception {
        if (request == null) {
            request = new GuiaDespachoRequest();
        }
        if (esVacio(request.getTransportista())) {
            request.setTransportista(transportista);
        }
        if (esVacio(request.getNumeroPedido())) {
            request.setNumeroPedido("PEDIDO-" + limpiar(idGuia));
        }
        if (esVacio(request.getDestinatario())) {
            request.setDestinatario("Destinatario actualizado");
        }
        if (esVacio(request.getDireccionDestino())) {
            request.setDireccionDestino("Dirección actualizada");
        }
        if (esVacio(request.getDescripcionCarga())) {
            request.setDescripcionCarga("Carga actualizada");
        }

        String key = construirKey(fecha, limpiar(transportista), limpiar(idGuia));
        byte[] pdf = generarPdfSimple(limpiar(idGuia), request, fecha);
        File archivoEfs = efsService.saveBytesToEfs(key, pdf);
        awsS3Service.uploadBytes(bucket, key, pdf, "application/pdf");
        return new GuiaDespachoResponse(idGuia, bucket, key, archivoEfs.getAbsolutePath(),
                "Guía modificada y actualizada correctamente");
    }

    public GuiaDespachoResponse crearGuiaDemo(String transportista) throws Exception {
        GuiaDespachoRequest request = new GuiaDespachoRequest();
        request.setNumeroPedido("PED-1001");
        request.setTransportista(esVacio(transportista) ? "transportistaX" : transportista);
        request.setUsuarioAutorizado(request.getTransportista());
        request.setDestinatario("Cliente Demo");
        request.setDireccionDestino("Av. Siempre Viva 123");
        request.setDescripcionCarga("Pedido de prueba generado por el microservicio");
